...
export default function PinPriceChart({ performance }: Props) {
  const yearly = [...new Set(performance.map(p => p.year))].sort().map(yr => {
    ...
    return {
      year: yr.toString(),
      '≤€6.99': yp.reduce((s, p) => s + p.plan_pin_699_below, 0),
      '>€6.99': yp.reduce((s, p) => s + p.plan_pin_699_above, 0),
    }
  })

  return (
    <div className="card">
      <h3 className="section-title mb-0.5">P-IN ≤€6.99 vs P-IN &gt;€6.99</h3>
      <p className="section-subtitle mb-4">Port-In plan comparison by price tier</p>
      <ResponsiveContainer width="100%" height={250}>
        <BarChart data={yearly} margin={{ top: 10, right: 20, bottom: 0, left: 0 }}>
          ...
          <Bar dataKey="≤€6.99" fill="#46286E" radius={[4, 4, 0, 0]} maxBarSize={50} />
          <Bar dataKey=">€6.99" fill="#21264E" radius={[4, 4, 0, 0]} maxBarSize={50} />
        </BarChart>
      </ResponsiveContainer>
    </div>
  )
